...
class UNet3D(nn.Module):
    def __init__(self, channel_factor=1) -> None:
        ...
        self.level4_conv1 = self.ConvBlock(in_channels=64*channel_factor, out_channels=64*channel_factor)
        self.level4_conv2 = self.ConvBlock(in_channels=64*channel_factor, out_channels=128*channel_factor)
        ...
        self.linear = nn.Sequential(OrderedDict([
            ("Linear1", nn.Linear(in_features=64*channel_factor + 1 + 3, out_features=32)),
    ...
    def forward(self, img, sex, age):
        ...
        img_feature_1d = torch.reshape(img_feature, (-1, 64*self.channel_factor))
...
class AttentionUNet3D(nn.Module):
    def __init__(self, img_channel=2, output_channel=1):
        ...
        self.level1_down = nn.Sequential()
        self.level1_down.add_module("level1_down_conv1", self.ConvBlock(img_channel, channels[0]))
        self.level1_down.add_module("level1_down_conv2", self.ConvBlock(img_channel, channels[0]))

        self.level2_down = nn.Sequential()
        self.level2_down.add_module("level2_down_conv1", self.ConvBlock(channels[0], channels[1]))
        self.level2_down.add_module("level2_down_conv2", self.ConvBlock(channels[1], channels[1]))

        self.level3_down = nn.Sequential()
        self.level3_down.add_module("level3_down_conv1", self.ConvBlock(channels[1], channels[2]))
        self.level3_down.add_module("level3_down_conv2", self.ConvBlock(channels[2], channels[2]))

        self.level4_down = nn.Sequential()
        self.level4_down.add_module("level4_down_conv1", self.ConvBlock(channels[2], channels[3]))
        self.level4_down.add_module("level4_down_conv2", self.ConvBlock(channels[3], channels[3]))

        self.level4_Up = self.UpSampling(channels[3], channels[2])
        self.Att4 = AttentionUNetblock(F_g=channels[2], F_l=channels[2], F_int=channels[1])
        self.Up_conv4 = self.ConvBlock(channels[3], channels[2])
