...
def training(
        ...
        cls_first_train_loader: DataLoader,
        cls_first_test_loader: DataLoader,
        seg_first_train_loader: DataLoader,
        seg_first_test_loader: DataLoader,
        writer: dict,
    ...
    with torch.enable_grad():
        ...
        for epoch in range(int(EPOCHS)):
            ...
            if epoch == 1:
                loop = tqdm(cls_first_train_loader, total=len(cls_first_train_loader))
                val_train_loader = cls_first_train_loader
                val_test_loader = cls_first_test_loader
            else:
                loop = tqdm(cls_train_loader, total=len(cls_train_loader))
                val_train_loader = cls_train_loader
                val_test_loader = cls_test_loader
            ...
            cls_val(cls_model, val_train_loader, cls_loss, tqdm_description="train_CAM", device=device)      # save training CAM

            if max_acc < accuracy:
                max_acc = accuracy
                early_stop = epoch
                torch.save(cls_model, os.path.join('/data/orfu/DeepLearning/Segmentation-Classification/deep learning/pipeline/model/stage2/pipeline/cls', f"{cls_model_name}_{tag}_best.pt"))
            ...
            if epoch == 1:
                loop = tqdm(seg_first_train_loader, total=len(seg_first_train_loader))
            else:
                loop = tqdm(seg_train_loader, total=len(seg_train_loader))
                
            val_train_loader = seg_train_loader
            val_test_loader = seg_test_loader
            ...
            seg_val(seg_model, val_train_loader, seg_loss, "train_Seg", model_name, device=device)
            
            if max_dice < dice:
                max_dice = dice
                early_stop = epoch
                torch.save(seg_model, os.path.join('/data/orfu/DeepLearning/Segmentation-Classification/deep learning/pipeline/model/stage2/pipeline/seg', f"{seg_model_name}_{tag}_best.pt"))

            # checkpoint
            # if epoch % 5 == 0:
            torch.save(cls_model, os.path.join('./pipeline/model/stage2/pipeline/checkpoint/cls', f"{cls_model_name}_{epoch}_01"+'.pt'))
            torch.save(seg_model, os.path.join('./pipeline/model/stage2/pipeline/checkpoint/seg', f"{seg_model_name}_{epoch}_01"+'.pt'))
        ...
        for data in loop:
            ...
            for cam, path in zip(activation_map, paths):
                if not os.path.exists(path):
                    os.makedirs(path)
                torch.save(cam, os.path.join(path, 'cam', 'stage2_cam.pt'))
    ...
    with torch.no_grad():
        ...
        for data in loop:
            ...
            for seg, path in zip(segmentation_01, paths):
                ...
                if not os.path.exists(path):
                    os.makedirs(path)
                torch.save(seg, os.path.join(path, model_name + '_stage2_01_mask.pt'))
            for seg, path in zip(segmentation, paths):
                seg = torch.squeeze(seg)
                path = os.path.join(path, 'predict_mask_pt')
                if not os.path.exists(path):
                    os.makedirs(path)
                torch.save(seg, os.path.join(path, model_name + '_stage2_sigmoid_mask.pt'))
            total_loss += segloss
...
def get_models(
    ...
    cls_model = torch.load(f"/data/orfu/DeepLearning/Segmentation-Classification/deep learning/pipeline/model/stage2/pretrain/cls/{cls_model}_{tag}_best.pt").to(device)
    seg_model = torch.load(f"/data/orfu/DeepLearning/Segmentation-Classification/deep learning/pipeline/model/stage2/pretrain/seg/{seg_model}_{tag}_best.pt").to(device)
...
class MyData(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == "cls":
            # mask_path = os.path.join(patient_dir, 'predict_mask_pt', self.model_name + '_stage2_sigmoid_mask.pt')
            mask_path = os.path.join(patient_dir, 'predict_mask_pt', self.model_name + '_stage2_01_mask.pt')
            ...
            cam_path = os.path.join(patient_dir, 'cam', 'stage2_cam.pt')
...
def main():
    ...
    if not train_for_best:
        grid_search_path = os.path.join("./pipeline/Performance/stage2", f"{cls_model_name}_{seg_model_name}_{modality}")
        ...
        for combination in combinations:
            ...
            paths = [
                os.path.join('./pipeline/log/stage2/train', f"{cls_model_name}_{seg_model_name}_{modality}", param_path, "val"+str(validation)+"both"),
                os.path.join('./pipeline/log/stage2/test', f"{cls_model_name}_{seg_model_name}_{modality}", param_path, "val"+str(validation)+"both"),
            ]
            ...
            cls_first_train_data = MyData(classes, trainlist, 'cls', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
            cls_first_train_dataloader = DataLoader(dataset=cls_first_train_data, batch_size=cls_batch, shuffle=True, drop_last=False, num_workers=cls_nums_worker, persistent_workers=cls_persistent_workers)
            seg_first_train_data = MyData(classes, trainlist, 'seg', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
            seg_first_train_dataloader = DataLoader(dataset=seg_first_train_data, batch_size=seg_batch, shuffle=True, drop_last=False, num_workers=seg_nums_worker, persistent_workers=seg_persistent_workers)
            
            cls_first_valid_data = MyData(classes, [validation], 'cls', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
            cls_first_valid_dataloader = DataLoader(dataset=cls_first_valid_data, batch_size=cls_batch, shuffle=False, drop_last=False, num_workers=cls_nums_worker, persistent_workers=cls_persistent_workers)
            seg_first_valid_data = MyData(classes, [validation], 'seg', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
            seg_first_valid_dataloader = DataLoader(dataset=seg_first_valid_data, batch_size=seg_batch, shuffle=False, drop_last=False, num_workers=seg_nums_worker, persistent_workers=seg_persistent_workers)
            ...
            early_stop, cls_model, seg_model, _, _, max_acc, max_dice = training(
                ...
                cls_first_train_dataloader,
                cls_first_valid_dataloader,
                seg_first_train_dataloader,
                seg_first_valid_dataloader,
    ...
    # retrain for best
    try:
        grid_search_path = os.path.join(f"./pipeline/Performance/stage2", f"{cls_model_name}_{seg_model_name}_{modality}")
        validation_df = pd.read_csv(os.path.join(grid_search_path, "grid search.csv"), index_col=0)
    ...
    param_path = "_".join([str(item) for key_value in params.items() for item in key_value])
    paths = [
        os.path.join('./pipeline/log/stage2/train', f"{cls_model_name}_{seg_model_name}_{modality}", param_path, "best_01"+"both"),
        os.path.join('./pipeline/log/stage2/test', f"{cls_model_name}_{seg_model_name}_{modality}", param_path, "best_01"+"both"),
        os.path.join("./pipeline/Performance/stage2", f"{cls_model_name}_{seg_model_name}_{modality}", param_path, "best_01"+"both"),
    ]
    for dir in paths:
        if not os.path.exists(dir):
            os.makedirs(dir)
    ...
    cls_first_train_data = MyData(classes, list(range(1, kFold)), 'cls', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
    cls_first_train_dataloader = DataLoader(dataset=cls_first_train_data, batch_size=cls_batch, shuffle=True, drop_last=False, num_workers=cls_nums_worker, persistent_workers=cls_persistent_workers)
    seg_first_train_data = MyData(classes, list(range(1, kFold)), 'seg', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
    seg_first_train_dataloader = DataLoader(dataset=seg_first_train_data, batch_size=seg_batch, shuffle=True, drop_last=False, num_workers=seg_nums_worker, persistent_workers=seg_persistent_workers)

    cls_first_test_data = MyData(classes, [0], 'cls', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
    cls_first_test_dataloader = DataLoader(dataset=cls_first_test_data, batch_size=cls_batch, shuffle=False, drop_last=False, num_workers=cls_nums_worker, persistent_workers=cls_persistent_workers)
    seg_first_test_data = MyData(classes, [0], 'seg', model_name=model_name, first_epoch=True, device=device, root_dir=root_dir, modality=modality)
    seg_first_test_dataloader = DataLoader(dataset=seg_first_test_data, batch_size=cls_batch, shuffle=False, drop_last=False, num_workers=seg_nums_worker, persistent_workers=seg_persistent_workers)
    ...
    _, cls_model, seg_model, _, _, max_acc, max_dice = training(
            ...
            seg_test_dataloader,
            cls_first_train_dataloader,
            cls_first_test_dataloader,
            seg_first_train_dataloader,
            seg_first_test_dataloader,
            model_name=model_name,
            cls_model_name="ResNet3D",
            seg_model_name="VNet3D",
            writer={"train": train_writer, "test": test_writer}, 
    ...
    # test_dice = seg_val(seg_model, seg_test_dataloader, "test_Seg", seg_loss)
    # print("get seg train result")
    # train_dice = seg_val(seg_model, seg_train_dataloader, "train_Seg", seg_loss)
